...
\begin{lemma}
	Let $\mathfrak{p} \in V(S)$ and suppose that $\mathfrak{q} \supseteq \mathfrak{p}$.
	Then $\mathfrak{q} \in V(S)$.
\end{lemma}
\begin{proof}
	$\mathfrak{p} \in V(S)$ if and only if $S \subseteq \mathfrak{p}$ so $S
	\subseteq \mathfrak{q}$ whence $\mathfrak{q} \in V(S)$.
\end{proof}

\begin{lemma}
	Irreducible subsets of $\Spec(R)$ are in one to one correspondence 
	with points via $\mathfrak{p} \mapsto V(\mathfrak{p})$.
\end{lemma}
\begin{proof}
	First, if $\mathfrak{p} \in \Spec(R)$ is a prime ideal, then
	$V(\mathfrak{p})$ is an irreducible subset of $\Spec(R)$ since if $V(S_1)
	\cup V(S_2) \supseteq V(\mathfrak{p})$ are two closed subsets covering
	$V(\mathfrak{p})$, one of $V(S_i)$ must contain the point $\mathfrak{p} \in
	V(\mathfrak{p})$, whence $V(S_i)$ contains any prime ideal containing
	$\mathfrak{p}$, I.e all of $V(\mathfrak{p})$. \\

	Now suppose that $V(S) \subset \Spec(R)$ is an irreducible subset. Let $J =
	\sqrt{(S)}$ be the radical ideal generated by $S$. Then $V(S) = V(J)$. Let
	$fg \in J$ with $f \not \in J$. $V(J)$ being irreducible means that either
	$D(f)$ or $D(g)$ intersects $V(J)$ trivially, whence $f$ or $g$ is in $J$,
	or they are two non-trivial open subsets of $V(J)$, whence they intersect
	non-trivially and we have some $\mathfrak{p} \in D(f) \cap D(g)$ such that
	$\mathfrak{p} \supseteq J$. But then $fg \in \mathfrak{p}$ whilst $f, g
	\not \in \mathfrak{p}$, a contradiction! Thus $J$ is prime and $V(S) =
	V(J)$.
\end{proof}
...
Now, if $R \subseteq S$ and $\mathfrak{p} \subset S$ is a prime ideal,
we have $\mathfrak{p} \cap R$ a prime ideal. Hence every irredu
